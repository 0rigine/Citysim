...
void valeur(city * ville)
{
    ...
    {
            Locate(130,0);printf("|");
    Locate(130,1);printf("|\t RESSOURCES DE LA VILLE : "); color(10,0);printf("%s",ville->nomdelaville);color(15,0);printf("[");color(11,0);printf("%s",ville->faction);color(15,0);printf("]");
    Locate(130,2);printf("|");
    Locate(130,3);printf("|");
...
void choice_inspect(int * x2, int * y2,int taille_x, int taille_y,city ** tab, const int taille)
{   int curseur_x=0,curseur_y=0,x=2,y=1;
    city * tampon = NULL;
      ...
      while (1) // gauche 0x4B   droite 0x77 haut 0x50  bas 0x48
    {
        ...
        if (touche == 0x50 && curseur_y < taille_y-1) // haut
        {
            curseur_y++;
            y = 4*curseur_y + 1;
            tampon = search_city_by_position(tab,80,x,y);
            valeur(tampon);
        }
        if (touche == 0x48 && curseur_y > 0) // bas
        {
...
void contract_ville(city * ville_A,city * ville_B,contract c)
{
    Locate(100,26);printf("Contrat de la ville : ");color(10,0);printf("%s",ville_B->nomdelaville);color(15,0);printf("[");color(11,0);printf("%s",ville_B->faction);color(15,0);printf("]");
    Locate(100,27);printf("Celle-ci vous propose un contract actif : ");color(10,0);printf("%d ",c.timeleft);color(15,0);printf("tour(s)");
    Locate(100,28);color(14,0);printf(" GOLD  ");color(15,0);printf("  : ");
    if (c.gold_purpose > 0)
    {
        color(10,0);printf(" + ");
    }
    ...
    printf("%d",ville_B->gold);color(15,0);
    Locate(100,29);color(12,0);color(12,0);printf(" BOUFFE ");color(15,0);printf(" : ");
    if (c.bouffe_purpose > 0)
    {
        color(10,0);printf(" + ");
    }
    ...
    printf("%d",ville_B->bouffe);color(15,0);
    Locate(100,30);color(9,0);printf(" ENERGIE");color(15,0);printf(" : ");
    if (c.energy_purpose > 0)
    {
         color(10,0);printf(" + ");
    }
    else
    ...
    printf("%d",ville_B->energy);color(15,0);

    const char* tab_reponse_oui_non[] = {"Oui !","Non, je prefere attendre..."};
    int d = Choix(tab_reponse_oui_non,2,105,32);
    switch (d)
    {
        case 1 :  break;
        ...
        while (d != 5)
        {
          ...
          d = Choix(tab_actions,5,25,28);
            switch(d)
            {
                ...
                case 2 :
                        choice_inspect(&x_choice,&y_choice,16,5,tab_city,80);
                        /*tampon = search_city_by_position(tab_city,80,x_choice,y_choice);
                        valeur(tampon); */

                        break;

                case 4 : contract_ville(test,test2,con);
                        erase(99,25,130,42);
                        break;
